...
for kk=1:n
    ...
    for ii=1:times
        ...
        %LDPC译码，采用LLR-MS译码算法
        [Bob_data,Bob_iter,~] = ldpcDecode(Bob_demodSignal.double,cfgLDPCDec,maxnumiter);
        %累计单次误比特率
        if cfgLDPCEnc.NumInformationBits==1040
        ...
        if ble(kk)>minble
            ...
            disp("iter:")
            disp(iter)
            save("error_floor_fixed.mat","minble","w","Ebn0","be","ble","ber","bler","ite","iter");
            break;
        end
    end
end
...
grid on
hold on
savefig("error_floor_fixed_ber.fig")
...
grid on
hold on
savefig("error_floor_fixed_bler.fig")
...
grid on
hold on
savefig("error_floor_fixed_iter.fig")